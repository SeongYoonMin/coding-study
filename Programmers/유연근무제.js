/**
Programmers Code Challenge 1차 예선 - 유연근무제
문제 설명
...
function solution(schedules, timelogs, startday) {
  ...
  // 모든 시각은 시에 100을 곱하고 분을 더한 정수(ex. 10시 13분은 1013이 되고 9시 58분은 958)
  // 직원 n
  // 출근 희망 시각 1차원 정수 배열 schedules
  // 일주일 동안 출근한 시각 2차원 정수 배열 timelogs
  // 이벤트를 시작한 요일을 의미하는 정수 startday
  // 1은 월요일, 2는 화요일, 3은 수요일, 4는 목요일, 5는 금요일, 6은 토요일, 7은 일요일에 이벤트를 시작
  let answer = 0;
  timelogs.forEach((el, index) => {
    const hope_time = schedules[index].toString();
    const work_min = hope_time.slice(-2); // 뒤에서 2개
    const work_hour =
      hope_time.length > 3 ? hope_time.slice(0, 2) : hope_time.slice(0, 1);
    const work_time = addTenTimes([work_hour, work_min]);
    let check = [];
    let day = startday;
    el.forEach((times) => {
      if (day > 5) {
        check.push(0);
      } else if (times <= work_time) {
        check.push(0);
      } else {
        check.push(1);
      }
